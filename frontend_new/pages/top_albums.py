...
from frontend_new.components.charts import build_top_albums_figure
from frontend_new.services.summary import cached_total_listening_seconds, format_duration_de
from frontend_new.services.top_albums import cached_top_albums

SCOPE = "top-albums"
dash.register_page(__name__, path="/top-albums", name="Top Alben")
...
def layout():
    ...
    right = [
        ...
        dbc.Card([
            dbc.CardHeader(
                dbc.Row([
                    dbc.Col(html.Div("Top Alben"), align="center"),
                    dbc.Col(metric_toggle(SCOPE), width="auto", align="center", className="ms-auto"),
                ], className="g-2")
            ),
            dbc.CardBody(
                dcc.Loading(id="tal-loading", type="default",
                            children=html.Div(id="tal-visual", children=skeleton_graph()))
            ),
        ])
    ]
...
def update_top_albums(n_apply, metric, start_date, end_date, top_n, theme, cache_ver):
    ...
    df = cached_top_albums(start_dt, end_dt, int(top_n or 30), metric, int(cache_ver or 0))

    dark = (theme == "dark")
    fig = build_top_albums_figure(df, metric, int(top_n or 30), dark)
    return dcc.Graph(id="tal-graph", figure=fig, config={"displayModeBar": True})
